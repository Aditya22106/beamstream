...
export default function SlideEditor() {
        ...
        <div className="w-full max-w-5xl mx-auto px-8">
          ...
          <div className="flex items-center justify-between mt-6">
            <button onClick={() => setCurrentIdx(p => Math.max(0, p-1))}
              ...
              className="px-6 py-3 bg-white/20 hover:bg-white/30 text-white rounded-xl disabled:opacity-30 transition">
              ← Previous
            </button>
            <div className="flex items-center gap-2">
              {slides.map((_, i) => (
                <button key={i} onClick={() => setCurrentIdx(i)}
                  className={`w-2.5 h-2.5 rounded-full transition ${i === currentIdx ? 'bg-yellow-400 scale-125' : 'bg-white/40'}`} />
              ))}
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f0f0f0] overflow-hidden">
      ...
      {/* Top bar */}
      <div className="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-3 flex-shrink-0 shadow-sm z-10">
        ...
        <input value={title} onChange={e => setTitle(e.target.value)}
          ...
          className="text-lg font-medium text-gray-800 bg-transparent border-none outline-none border-b-2 border-transparent focus:border-[#f4b400] transition py-1 w-56" />

        {session?.otp_code && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-1.5 text-center flex-shrink-0">
            <p className="text-[10px] text-yellow-600 font-semibold uppercase tracking-wider">Share OTP</p>
            <p className="text-lg font-black text-yellow-600 tracking-widest">{session.otp_code}</p>
          </div>
        )}

        <span className="text-xs font-medium flex-shrink-0 ml-2" style={{ color: STATUS_COLOR[saveStatus] }}>
          {STATUS_TEXT[saveStatus]}
        </span>

        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => setPresenting(true)}
            className="flex items-center gap-2 px-4 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-semibold rounded-lg transition">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-2 flex items-center gap-3 flex-shrink-0">
        <div className="flex items-center gap-1.5 text-xs text-gray-500"><Users size={13} /><span>{connUsers.length+1} editing</span></div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-yellow-100">
          <div className="w-5 h-5 rounded-full bg-yellow-500 flex items-center justify-center text-white text-[9px] font-bold">{getInitials(user?.name)}</div>
          <span className="text-xs text-yellow-800 font-medium">{user?.name}</span>
          <span className="text-[10px] text-yellow-600">● editing</span>
        </div>
        {connUsers.map(u => (
          <div key={u.user_id} className="flex items-center gap-1.5 px-2 py-1 rounded-full" style={{ background: u.color+'22' }}>
            <div className="w-5 h-5 rounded-full flex items-center justify-center text-white text-[9px] font-bold" style={{ background: u.color }}>{getInitials(u.name)}</div>
            <span className="text-xs font-medium" style={{ color: u.color }}>{u.name}</span>
          </div>
        ))}
        {updatedBy && <span className="ml-auto text-xs text-gray-400">Last edit: <b className="text-gray-600">{updatedBy}</b></span>}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-48 bg-gray-100 border-r border-gray-200 flex flex-col overflow-hidden flex-shrink-0">
            ...
            {slides.map((s, i) => (
              <div key={i} onClick={() => setCurrentIdx(i)}
                ...
                <div className="w-full aspect-video flex flex-col justify-center p-3 gap-1"
                  ...
                  <div className="text-[8px] font-bold text-gray-800 truncate">{s.title || `Slide ${i+1}`}</div>
                  <div className="text-[6px] text-gray-500 truncate">{s.content || ''}</div>
                </div>
                <div className="absolute bottom-1 left-1 text-[9px] text-gray-500 font-semibold">{i+1}</div>
                {slides.length > 1 && (
          ...
          <div className="p-3 border-t border-gray-200">
            <button onClick={addSlide}
              className="w-full flex items-center justify-center gap-1.5 py-2 bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-semibold rounded-lg transition">
        ...
        <div className="flex-1 overflow-auto flex flex-col items-center p-8 gap-4">
          ...
          <div className="w-full max-w-4xl rounded-lg shadow-lg overflow-hidden border border-gray-200"
            ...
            <div className="flex flex-col p-14 gap-6 min-h-[420px]">
              <div>
                <p className="text-[10px] text-gray-300 uppercase tracking-wider mb-1 font-semibold">Title</p>
              ...
              <div className="flex-1">
                <p className="text-[10px] text-gray-300 uppercase tracking-wider mb-1 font-semibold">Content</p>
          ...
          <div className="w-full max-w-4xl">
            ...
            <textarea value={slide.notes||''} onChange={e => updateSlide('notes', e.target.value)}
              ...
              className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm text-gray-600 resize-none focus:outline-none focus:border-yellow-400 transition font-[Arial]" />
          </div>
        </div>
              ...
              {PANEL_TABS.map(({ id, label }) => (
                <button key={id} onClick={() => setShowPanel(id)}
                  className={`flex-1 py-2.5 text-xs font-semibold transition
                    ${showPanel === id ? 'text-yellow-700 border-b-2 border-yellow-500 bg-yellow-50/50' : 'text-gray-400'}`}>
                  {label}
                </button>
              ))}
            </div>
              ...
              <div className="flex flex-col flex-1 overflow-hidden">
                    ...
                    : comments.map((c, i) => (
                      <div key={i} className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-1.5">
                          <div className="w-6 h-6 rounded-full bg-yellow-500 flex items-center justify-center text-white text-[9px] font-bold">{getInitials(c.author)}</div>
                ...
                <div className="p-3 border-t border-gray-100 flex-shrink-0">
                  <textarea value={newComment} onChange={e => setNewComment(e.target.value)}
                    ...
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg resize-none focus:outline-none focus:border-yellow-500 text-gray-700" />
                  <button onClick={postComment}
                    className="mt-2 w-full py-2 bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-semibold rounded-lg transition">
                    Post Comment
                  </button>
                </div>
              </div>
